...
pub fn solve(task_1_cipher: &PathBuf, out_folder: &PathBuf) -> Result<()> {
    ...
    let mut signer = Signer::new(MessageDigest::sha256(), &key_pair).context("Constructing signer")?;
    let signature = signer.sign_oneshot_to_vec(task1_sol.as_slice()).context("Signing")?;
    let signature = base64::encode_block(signature.as_slice());
    std::fs::write(out_folder.join(format!("Sig_{}.txt", id)), signature).context("Writing signature")?;
    Ok(())
}

fn load_and_decode_task_1_cipher(path: &PathBuf) -> Result<(String, Vec<u8>)> {
    let id = path.file_stem().context("Not a file")?.to_string_lossy().strip_prefix("Enc_").context("File does not start with 'Enc_'")?.into();
